    ...
    $( "#signup" ).click(function() {
       ...
       $("dialog button.close").click(function() {
        ...
        if (user=="" || user==undefined || user=="NULL"){            
           $('#dialag').html(dialag('Error!', "Si us plau, posa un camp de correu electrònic vàlid!"));
           $('.errosSignup').show();
           $('.errosSignup').css('z-index', '1');
        }else{success++;}
        if (pass=="" || pass==undefined || pass=="NULL") {
            $('#dialag').html(dialag('Error!', "Si us plau, posa un camp de contrasenya vàlid!"));
            $('.errosSignup').show();
            $('.errosSignup').css('z-index', '1');           
        }else{success++;}
        if(model=="" || model==undefined || model=="NULL"){
            $('#dialag').html(dialag('Error!', "Si us plau, selecciona un marca i model de cotxe vàlid!"));
            $('.errosSignup').show();
            $('.errosSignup').css('z-index', '1');   
        }else{success++;}
